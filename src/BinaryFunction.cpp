...
static cl::list<std::string> PrintOnly("print-only", cl::CommaSeparated,
...
void BinaryFunction::print(raw_ostream &OS, std::string Annotation,
  ...
  OS << "\n  Number      : " << FunctionNumber
     ...
     << Twine::utohexstr(MaxSize) << "\n  Offset      : 0x"
     << Twine::utohexstr(FileOffset) << "\n  Section     : " << SectionName
     << "\n  Orc Section : " << getCodeSectionName() << "\n  LSDA        : 0x"
     << Twine::utohexstr(getLSDAAddress()) << "\n  IsSimple    : " << IsSimple
...
BinaryFunction::processIndirectBranch(MCInst &Instruction, unsigned Size,
  ...
  auto Type = BC.MIB->analyzeIndirectBranch(
  ...
  auto useJumpTableForInstruction = [&](JumpTable::JumpTableType JTType) {
    ...
    BC.MIB->replaceMemOperandDisp(const_cast<MCInst &>(*MemLocInstr), JTLabel,
                                  BC.Ctx.get());
...
bool BinaryFunction::disassemble() {
  ...
  for (uint64_t Offset = 0; Offset < getSize(); Offset += Size) {
    ...
    for (auto Itr = Relocations.lower_bound(Offset),
      ...
      const auto Result = BC.MIB->replaceImmWithSymbolRef(
  ...
  for (auto &JTSite : JTSites) {
    ...
    while (EntryOffset < JT->getSize()) {
      ...
      if (BC.HasRelocations) {
        ...
        (void)Res;
        DEBUG(auto Section = BC.getSectionForAddress(EntryAddress);
...
void BinaryFunction::dumpGraphToTextFile() const {
  auto Filename = constructFilename(getPrintName(), "", ".txt");
  outs() << "BOLT-DEBUG: Dumping CFG to " << Filename << "\n";
  std::error_code EC;
...
void BinaryFunction::updateOutputValues(const MCAsmLayout &Layout) {
  ...
  const auto ColdBaseAddress = isSplit() ? ColdSection->getOutputAddress() : 0;
...
DWARFDebugLoc::LocationList BinaryFunction::translateInputToOutputLocationList(
  ...
  for (const auto &Entry : InputLL.Entries) {
    ...
    if (!containsAddress(Start)) {
      DEBUG(dbgs() << "BOLT-DEBUG: invalid debug address range detected for "
                   << *this << " : [0x" << Twine::utohexstr(Start) << ", 0x"
                   << Twine::utohexstr(End) << "]\n");
      continue;
    }
    ...
    do {
      ...
      if (const auto OutputStart = BB->getOutputAddressRange().first) {
        ...
        if (StartAddress == PrevEndAddress && Entry.Loc == *PrevLoc) {
          OutputLL.Entries.back().End =
              std::max(OutputLL.Entries.back().End, EndAddress);
        } else {
          OutputLL.Entries.emplace_back(DWARFDebugLoc::Entry{
              StartAddress, std::max(StartAddress, EndAddress), Entry.Loc});
        }
  ...
  std::stable_sort(
      OutputLL.Entries.begin(), OutputLL.Entries.end(),
  ...
  for (const auto &Entry : OutputLL.Entries) {
    if (Entry.Begin <= PrevEndAddress && *PrevLoc == Entry.Loc) {
      MergedLL.Entries.back().End =
          std::max(Entry.End, MergedLL.Entries.back().End);
    } else {
      const auto Begin = std::max(Entry.Begin, PrevEndAddress);
      const auto End = std::max(Begin, Entry.End);
      MergedLL.Entries.emplace_back(
          DWARFDebugLoc::Entry{Begin, End, Entry.Loc});
    }
